...
export default function EditVente({ vente, onClose, updateData }) {
  const [vins, setVins] = useState([]);
  const [venteData, setVenteData] = useState({
    date: vente.date,
    mode_paiement: vente.mode_paiement,
    client: vente.client.num_client,
    vins: vente.vins
  });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`http://localhost:3000/vente/${vente.num_vente}`, venteData);
    ...
    <div className='fixed top-0 left-0 w-full h-full flex justify-center items-center bg-gray-500 bg-opacity-50'>
      <div ref={dialogRef} className='rounded-xl bg-white w-full max-w-md md:max-w-lg lg:max-w-xl z-10'>
        <div className="flex p-3 mb-4 bg-charade-100 pb-4 rounded-t-xl ">
          <h2 className='text-lg md:text-xl font-semibold mr-auto  mx-2 text-charade-600'>Modifier une vente</h2>
